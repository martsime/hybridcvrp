...
mod tests {
    ...
    #[test]
    fn timeit() {
        let mut schedule = IterationSchedule::new(100.0, 1_000_000);
    ...
    fn timeit2() {
        ...
        let c = (0.0001 / 100.0 as f64).powf(1.0 / 1_000_000 as f64);
        ...
        panic!("{:?}: {:?}", start.elapsed(), t);
    }

    #[test]
    fn timeit3() {
        let mut schedule = TimeSchedule::new(100.0, Duration::from_millis(1000));
        schedule.update_rate = 100;

        let start = Instant::now();
        while schedule.valid() {
            schedule.update();
        }
        panic!("{:?}: {:?}", start.elapsed(), schedule.iterations);
    }

    #[test]
    fn timeit4() {
        let mut schedule = IterationSchedule::new(100.0, 1_000_000_000);

        let start = Instant::now();
        while start.elapsed().as_secs_f64() < 1.0 {
            schedule.update();
        }
        panic!("{:?}: {:?}", start.elapsed(), schedule.iteration);
    }
}
